...
// 1. localStorage d'abord (synchrone)
loadSavedConfig(Object.keys(CONFIG) as (keyof typeof CONFIG)[])

// 2. Preset "main" du serveur (async) → écrase localStorage si trouvé
async function applyMainPreset(): Promise<void> {
  try {
    const presets = await fetchPresets()
    const main = presets['main'] as Record<string, number> | undefined
    if (!main) return
    for (const [k, v] of Object.entries(main)) {
      if (typeof v === 'number' && k in CONFIG) {
        (CONFIG as Record<string, number>)[k] = v
      }
    }
  } catch { /* serveur absent (dev sans backend) → on garde localStorage */ }
}
